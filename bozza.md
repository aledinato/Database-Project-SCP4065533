...
I container possono avere associati più volumi che però sono allocati su un nodo, il container ha associato solo volumi che sono presenti nel nodo in cui il container è ospitato, almeno che il volume non sia di tipo globale, in quel caso il volume è allocato su un server esterno.  
...
Necessita oltre al path fisico del volume, anche dell'indirizzo IP del server esterno così da poterlo raggiungere.
...
Inoltre consideriamo che vengono creati in media 3 deployment al giorno (dev, test e produzione) con 50 servizi per deployment, quindi in totale ipotizziamo 1000 deployment come volume.  
  ...
  Come già precisato, specifichiamo i volumi della base di dati:  
